...
`2.0.1` 沿用 `2.0.0` 的字典和转换规则，优化文本拼接与分发体积，并提供原生 ESM 入口。已有首字母索引无需迁移。**`2.0.1` 已正式发布到 npm，可通过 `npm install pyfl` 安装。**
...
`2.0.1` npm 包同时包含原生 ESM 和 CommonJS/UMD 产物。Node ESM、浏览器模块和支持 `exports` 的构建器可以直接使用默认导出；TypeScript 声明为 `pyfl(raw: unknown): string`。本地构建方式见[从源码构建](#从源码构建)。
