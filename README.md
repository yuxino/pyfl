...
**版本说明：** 当前 `main` 为待发布的 `2.0.1`，官网使用这份源码构建。npm 上的 `2.0.0` 尚未包含本次体积、循环和原生 ESM 改进。
...
`2.0.1` 的源码构建额外提供原生 ESM 入口。Node ESM、浏览器模块和支持 `exports` 的构建器可以直接使用默认导出；TypeScript 声明为 `pyfl(raw: unknown): string`。
